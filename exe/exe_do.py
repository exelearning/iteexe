...
except ImportError as error:  # Python 3 syntax
...
def prepareParser():
    usage = _("%prog, the commandline version of eXe.\n"
    ...
    parser.add_option("-a", "--append",
                      ...
                      help=_("Append <input_file> elp package to <output_file> elp package").encode(ENCODING))
    
    parser.add_option("-s", "--set",
                      action="store", type='string', dest='set_options', metavar='field=value,<field=value>...',
                      help=_("Set ELP field values. Example: -s style=Default,root.title='Root Node'").encode(ENCODING))

    group = optparse.OptionGroup(parser,
                     _("Set ELP field values options").encode(ENCODING),
                     _("Once you have chosen Set ELP field values option using '-s <values>' or "
    ...
    group.add_option("-w", "--write_package",
                     ...
                     help=_("Write field values to ELP").encode(ENCODING))

    parser.add_option_group(group)

    parser.add_option("-r", "--report",
                      action="store_true", dest='report',
                      help=_("Generates resource info report to text <output_file> "
                             "from <input_file> elp package").encode(ENCODING))

    parser.add_option("-l", "--checker_logfile",
                      dest='checker_logfile', metavar='LOGFILE')

    parser.add_option("-x", "--export",
                      action="store", dest="x", metavar="format",
                      choices=['xml', 'scorm12', 'scorm2004', 'ims', 'website', 'webzip', 'singlepage',
                               'xliff', 'epub3', 'text'],
                      help=_("Export <input_file> elp package to optional "
    ...
    parser.add_option("-i", "--import",
                      ...
                      choices=['xml', 'xliff'],
                      help=_("Import to <output_file> elp package, <input_file> "
                             "in one of the given formats: xml or xliff.").encode(ENCODING))

    parser.add_option("-f", "--force",
                      action="store_true", dest="f",
                      help=_("Force overwrite of <output_file>").encode(ENCODING))

    parser.add_option("--editable",
                      action="store_true", dest="editable",
                      help=_("Add the required files to generate an export editable by eXeLearning").encode(ENCODING),
                      default=False)

    group = optparse.OptionGroup(parser,
                        _("XLIFF export options").encode(ENCODING),
                        _("Once you have chosen XLIFF export option using '-x xliff' or "
                          "'--export xliff', it's possible to configure the following export options:").encode(ENCODING))

    group.add_option("--no-copy-source",
                     action="store_false", dest="copy",
                     help=_("Don't copy source in target").encode(ENCODING), default=True)

    group.add_option("--wrap-cdata",
                     action="store_true", dest="wrap",
                     help=_("Wrap fields in CDATA").encode(ENCODING), default=False)
...
if __name__ == "__main__":
    ...
    if options.x and options.i:
        parser.error(_('Options --export and --import are mutually exclusive.').encode(ENCODING))
    if options.x and options.a:
        parser.error(_('Options --export and --append are mutually exclusive.').encode(ENCODING))
    if options.i and options.a:
        parser.error(_('Options --import and --append are mutually exclusive.').encode(ENCODING))

    if not options.a and not options.x and not options.i and not options.set_options and not options.report:
        parser.error(_('No --append --export, --import, --set or --report option supplied.').encode(ENCODING))

    if not args:
        parser.error(_('No file input supplied.').encode(ENCODING))

    if options.a:          
        if len(args) != 2:
            parser.error(_('Bad number of arguments supplied').encode(ENCODING))
        ...
        try:                
            ...
            pkg1.save()
            print(_("Successfully appended '%s' to '%s'.") % (args[0], args[1]))
        except Exception as e:
            print(_("exe_do: error: Unable to append elp"))
            print(e)
            sys.exit(1)
    else:
        inputf = args[0]
        ...
        if options.set_options:
            try:
                ...
                if not pkg:
                    raise Exception(_("Invalid input package").encode(ENCODING))
                ...
                print(_("exe_do: error: Unable to set values from '%s'.\nThe error was:") % path)
                print(e)
                sys.exit(1)
        ...
        if options.x:
            ...
            try:
                ...
                print(_("Successfully exported '%s' from '%s'.") % (outputf, inputf))
            except Exception as e:
                print(_("exe_do: error: Unable to export from '%s'.\nThe error was:") % inputf)
                print(e)
        ...
        if options.i:
            ...
            try:
                ...
                print(_("File '%s' successfully imported to '%s'.") % (inputf, outputf))
            except Exception as e:
                print(_("exe_do: error: Unable to import '%s'.\nThe error was:") % inputf)
                print(e)
