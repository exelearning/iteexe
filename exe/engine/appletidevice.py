...
class AppletIdevice(Idevice):
    ...
    def __init__(self, parentNode=None):
        ...
        self.appletCode        = u""
        self.type              = u"other"
        self._fileInstruc      = _(u"""Add all the files provided for the applet
...
files, once loaded will be displayed beneath the Applet code field.""")
        self._codeInstruc      = _(u""""Find the .txt file (in the applet file) 
and open it. Copy the contents of this file <ctrl A, ctrl C> into the applet 
code field.""")
        self._typeInstruc     = _(u""" <p>If the applet you're adding was generated 
...
you created in Geogebra.</p>""")
    ...
    def copyFiles(self):
        ...
        if self.type == "geogebra":
            ...
            self.message       = ""
            self._typeInstruc  = _(u"""Clic on AddFiles button for select the .ggb file and clic on Upload button after.""")
        if self.type == "jclic":
            #from exe.application import application
            from exe import globals
            ideviceDir = globals.application.config.webDir/'templates'            
            ...
            self.message       = ""
            self._typeInstruc  = _(u"""Clic on AddFiles button for select the .jclic.zip file and clic on Upload button after.<p>The activity will be visible when the HTML file will be generated from eXe.""")
        if self.type == "scratch":
            #from exe.application import application
            from exe import globals
            ideviceDir = globals.application.config.webDir/'templates'            
            ...
            self.message       = ""
            self._typeInstruc  = _(u"""Clic on AddFiles button for select the .sb or .scratch file and clic on Upload button after.""")
        if self.type == "descartes":
            #from exe.application import application
            from exe import globals
            ideviceDir = globals.application.config.webDir/'templates'            
            ...
            self.message       = ""
            self._typeInstruc  = _(u"""Please write: scene number,URL (no spaces) that include it, eg: 3,http://example.com; clic on Upload button after.""")
