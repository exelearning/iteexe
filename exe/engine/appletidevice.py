...
you created in Geogebra.</p>""")
                    ...
                    # tipo = macro or makro finded, now we need expresion parameter inside value tag                               
                        ...
                        for y in key_macro:
                            if string.find(tag, y) != -1:
                                ...
                                wexpression = r"%s'(?P<bla>[\w./]+)'" % y # notice unicode conversion
    ...
    def copyFiles(self):
        ...
        if self.type == "geogebra":
            ...
            self.message       = ""
            self._typeInstruc  = x_("""Click on the AddFiles button to select the .ggb file and then click on the Upload button.""")
        if self.type == "jclic":
            #from exe.application import application
            from exe import globals
            ideviceDir = globals.application.config.webDir/'templates'            
            ...
            self.message       = ""
            self._typeInstruc  = x_("""Click on the AddFiles button to select the .jclic.zip file and then click on the Upload button.<p>The activity will be visible when the HTML file will be generated from eXe.""")
        if self.type == "scratch":
            #from exe.application import application
            from exe import globals
            ideviceDir = globals.application.config.webDir/'templates'            
            ...
            self.message       = ""
            self._typeInstruc  = x_("""Click on the AddFiles button to select the .sb or .scratch file and then click on the Upload button.""")
        if self.type == "descartes":
            #from exe.application import application
            from exe import globals
            ideviceDir = globals.application.config.webDir/'templates'            
            ...
            self.message       = ""
            self._typeInstruc  = x_("""Please enter: scene number,URL (no spaces) that include it, eg: 3,http://example.com; click on the Upload button afterwards.""")
        if self.type == "other":
            self._typeInstruc  = x_("""Click on the AddFiles button to select the .html or .htm file and then click on the Upload button to get its content.""")
