...
class eXeRequest(appserver.NevowRequest):
    def gotPageContext(self, pageContext):
        ...
        if session.initialConfig:
            locale = getLocaleFromRequest(request)
            session.config.locale = locale
            session.config.configParser.set('user', 'locale', locale)
            session.initialConfig = False
        if session.config:
            G.application.config = session.config
            session.site.server.application.config = session.config
            session.site.server.preferences.config = session.config
            session.config.locales[session.config.locale].install(unicode=True)
        if session.ideviceStore:
            G.application.ideviceStore = session.ideviceStore
...
class eXeSession(server.Session):
    ...
    def setUser(self, user):
        config_path = G.application.config.configDir / user
        if config_path.exists():
            self.initialConfig = True
        self.config = UserConfig(config_path)
