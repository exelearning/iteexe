	...
	TooltipDialog = {
				...
				if (c) {
					...
					if (c.length==3) { // In case there are different versions of the plugin
						...
						if (clr!="" && rnd!="" && shd!="") {
							...
							if (rnd=="true") win.find('#longtext')[0].checked(true);
							if (shd=="true") win.find('#longtext')[0].checked(true);
							TooltipDialog.setPreview();
						}
					}
				}
		...
		cookie : {
			...
			check : function(cookieName) {
				var cookieValue=TooltipDialog.cookie.check(cookieName);
				if (cookieValue != "") {
					alert("cookieValue: " + cookieValue);
				} else {
				   // setCookie("username", user, 30);
				   alert("Establezco valor cookie")
				}
			}
		...
		setAttribs : function(elm,type) {
			...
			var ref = document.getElementById("href").value.replace(/ /g, '%20');
		...
		insert : function(content){
			...
			} else TooltipDialog.setAttribs(elm,type);
			...
			if (type=="type2") {
				...
				var id = "";
				if (elm == null) {
					if (typeof(this.currentLinkId)!='undefined') id = this.currentLinkId;
				} else {
