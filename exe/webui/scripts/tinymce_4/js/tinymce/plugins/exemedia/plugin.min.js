...
tinymce.PluginManager.add('exemedia', function(editor, url) {	
	...
	// eXeLearning
	var emBase = "https://mediateca.educa.madrid.org/";
	// / eXeLearning
	...
	var urlPatterns = [
		...
		// eXeLearning
		{regex: /mediateca\.educa\.madrid\.org\/(.*)\/([^_]+)/, type: 'iframe', w: 560, h: 314, url: 'https://mediateca.educa.madrid.org/$1/$2/fs', allowFullscreen: true}
		// / eXeLearning
	];
	...
	function parse_media_html_attributes(c) {
		...
		*/
		...
		var div = jQuery("<div></div>");
			div.html(new_c);
		jQuery("video,audio,iframe",div).each(function(){
					...
					items: [
						...
						},
						{
							...
							items: [
								...
								{name: 'jsplayer', type: 'checkbox', checked: false, text: _("JavaScript player"), tooltip: _("Old browsers compatibility with some formats. Not needed in modern browsers.")},
	...
	function submitForm(win) {
		...
		// allowfullscreen was removed in some cases (see #706)
		html = html.replace('allowfullscreen="allowfullscreen"','exe-allowfullscreen="exe-allowfullscreen"')
		...
		html = html.replace('exe-allowfullscreen="exe-allowfullscreen"','allowfullscreen="allowfullscreen"');
	...
	function getPreviousValues(win){
		...
		if (jsplayer && jsplayer.indexOf('mediaelement')!=-1) win.find("#jsplayer")[0].checked(true);
	...
	function dataToHtml(data,isDataEmbed) { // isDataEmbed is optional
		...
		// eXeLearning (mediateca.educa.madrid.org)
		if (data.source1.indexOf("https://mediateca.educa.madrid.org/")==0) {
			var tmp = data.source1;
				tmp = tmp.split("/");
				if (tmp.length>4) {
					tmp = "https://mediateca.educa.madrid.org/"+tmp[3]+"/"+tmp[4];
					data.source1 = tmp;
				}
		}
		// / eXeLearning
		...
		var testURL = data.source1;
		
		// mediateca.educa.madrid.org (audio size)
		if (testURL.indexOf("https://mediateca.educa.madrid.org/")==0) {
			var elm = editor.selection.getNode();
			if (testURL.indexOf("/audio/")!=-1) {
				// Update new audio size
				if (!(elm&&typeof(elm.className)=='string'&&elm.className=="mce-preview-object mce-object-iframe")) {
					if (data.width==560&&data.height==314) { // Audios have a different size
						data.width = 420;
						data.height = 44;
					}
				}
		...
		if (data.type=="embed") {
			if (testURL.startsWith("resources/")&&testURL.endsWith(".pdf")) {
				isLocalPDF = true;
			}
		} else if ((data.type==""||data.type=="embed")&&testURL.startsWith("/previews/")&&testURL.endsWith(".pdf")) {
			data.type = "embed";
		...
		if (isDataEmbed!=false && data.embed) {
			...
			if (data.type == "iframe") {
				...
				var allowFullscreen = (data.allowFullscreen=="true") ? ' allowFullscreen="1"' : '';
				// / eXeLearning
			...
			// eXeLearning
				...
				html += (
					'<embed src="'+data.source1+'" type="application/pdf" width="'+data.width+'" height="'+data.height+'" />'
				);
			// / eXeLearning
			} else {
				html = (
		...
		if (testURL.indexOf("https://mediateca.educa.madrid.org/")==0&&testURL.indexOf("/audio/")!=-1) {
			html = html.replace(' allowfullscreen="allowfullscreen"','');
		}
		html = html.replace(' data-mce-fragment="1"',''); // Remove data-mce-fragment attr
		// / eXeLearning
