		...
		win = editor.windowManager.open({
			...
			onSubmit: function() {
				...
				// editor.insertContent(dataToHtml(this.toJSON()));
					...
					// Is in figure?
					var figure = editor.dom.getParents(editor.selection.getNode(), '.exe-figure');
					if (figure.length==1) {
	...
	function getPreviousValues(win){
		...
		if (figure.length==1) {
			...
			if (license.length==1) {
				...
				var licenseText = license.text().replace("(","").replace(")","");
				...
				// CC-BY, CC-BY-SA, CC-BY-ND, CC-BY-NC, CC-BY-NC-SA, CC-BY-NC-ND								
					...
					licenseField.value(licenseText.replace(/ /g,'-'));
				} 
				// GNU/GPL
				...
				// All Rights Reserved
				else if (licenseText==_("All Rights Reserved")) {
