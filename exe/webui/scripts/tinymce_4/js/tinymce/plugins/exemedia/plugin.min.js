...
tinymce.PluginManager.add('exemedia', function(editor, url) {	
	...
	var oggIsAudio = false;
	var eXeMediaDialog = {};
	var allowedFormats = "mp4, mp3, webm, ogg/ogv, mpeg, swf, flv";
	...
	function guessMime(url) {
		...
		if (url.indexOf('.ogg') != -1) {
			if (oggIsAudio) return 'audio/ogg';
			else return 'video/ogg';
		}
	...
	function showDialog() {
		...
		win = editor.windowManager.open({
			...
			onSubmit: function() {
				oggIsAudio = false;
				eXeMediaDialog = this;
				if (win.find("#source1")[0].value().indexOf("ogg")!=-1) {
					editor.windowManager.confirm(
						_('OGG is valid for audio and video.\n"Cancel" to insert a video. "Ok" to insert an audio.'),
						function(s){
							if (s) {								
								oggIsAudio = true;
								submitForm(eXeMediaDialog);
							} else {
								submitForm(eXeMediaDialog);
							}
						}
	...
	function submitForm(win) {
		...
		if (selectedFileExt=="mov") {
            ...
            if (document.getElementById('mcemediasource').value.toLowerCase().indexOf("<object")!=-1) {
				editor.windowManager.alert(_('The content will be updated, but please convert the video to another format:') + ' webm, mp4, ogg...');
				...
				editor.windowManager.alert(
					...
					_('Please use any of these formats or provide a link to the file itself: ') + allowedFormats + '.'
				);
				return;
			}
				...
				function(s){
					if (s) {								
						...
						if (source!='') {
							...
							var newSource = '<video width="320" height="240" class="mediaelement" controls="controls">';
							...
							editor.windowManager.alert(_('Could not retrieve data (Core error)'),function(){
								editor.windowManager.close();
							});
						}
