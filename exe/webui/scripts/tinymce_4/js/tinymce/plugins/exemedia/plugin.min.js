...
tinymce.PluginManager.add('exemedia', function(editor, url) {	
			...
			body: [
				{
					...
					items: [
						...
						},
						{
							...
							spacing: 5,
							items: [
								{name: 'jsplayer', type: 'checkbox', checked: true, text: _("JavaScript player")},
			...
			if (data.type == "iframe") {
				...
				html = (
					'<video width="' + data.width + '" height="' + data.height + '"' + (data.poster ? ' poster="' + data.poster + '"' : '') + ' controls="controls">\n' +
						...
						(data.sourcesbt && data.sourcesbt !== "" ? '<track label="' + _("Subtitles") + '" kind="subtitles" srclang="" src="' + data.sourcesbt + '" default />\n' : '') +
					'</video>'
				);
			}
		}
	...
	function updateHTMLSBT(code,sbt){
		...
		jQuery("video",div).each(function(){
			...
			if(!hassbt){
				$media.append('<track label="' + _("Subtitles") + '" kind="subtitles" srclang="" src="' + sbt + '" default />')
				return div.html();
			}
		});
		return div.html();
