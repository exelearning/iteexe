...
tinymce.PluginManager.add('modalwindow', function(editor, url) {
		...
		insert : function(content){
			...
			if (typeof(jQuery)=='function') {
				...
				if (dt.length==1) {
					...
					if (linkText!="") {
						var s = '<style type="text/css">#'+id+':before{content:"'+inst.translate("Modal Dialog Window")+' - '+linkText+'"}</style>';
	...
	editor.on('init', function(e) {
		...
		// jQuery is required
		if (typeof(jQuery)!='function') {
			alert(tinymce.translate('Modal Dialog Window')+": "+tinymce.translate('jQuery is required'));
			return false;			
		}
		...
		var s = '<style type="text/css">';
			// Default title
			s += '.exe-dialog-text:before{content:"'+tinymce.translate('Modal Dialog Window')+'"}';
			...
			jQuery(".exe-dialog-text",d).each(function(){
				...
				if (dt.length==0) {
					// Content not found
					s += '#'+this.id+':before{content:"'+tinymce.translate('Modal Dialog Window')+' - '+tinymce.translate("Link not found")+'"}';
				} else if (dt.length==1) {
					var content = dt.text();
					if (content!="") s += '#'+this.id+':before{content:"'+tinymce.translate('Modal Dialog Window')+' - '+content+'"}';
				}
			});				
			s += '</style>';
		jQuery("HEAD",d).append(s);	
