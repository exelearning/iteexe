	...
	function showDialog(imageList) {
		...
		if (editor.settings.image_advtab) {
			...
			win = editor.windowManager.open({
				...
				onSubmit: function(){
					// Return the preview basename of img
					var previews_img = win.find("#src")[0].value()
					var previews_base = previews_img+".exe_info"
					var previewBasename = function() {
						let xhp = new XMLHttpRequest();
						xhp.onload = function() {
							xhp.basename = null;
							if (xhp.status != 404 && xhp.readyState === xhp.DONE && xhp.status === 200) {
								xhp.basename = xhp.response;
							} else {
								return false;
							}
						}
						xhp.open('GET', previews_base, false);
						xhp.send();
						if (xhp.basename) {
							return xhp.basename.split("=")[1];
						} else {
							return false;
						}
					}
					// Return the resource list of package
					var resources_url = "resources";
					var resourceList = function() {
						let xhr = new XMLHttpRequest();
						xhr.onload = function() {
							xhr.resources = [];
							if (xhr.status != 404 && xhr.readyState === xhr.DONE && xhr.status === 200) {
								parser = new DOMParser();
								xmlDoc = parser.parseFromString(xhr.response,"text/xml");
								aalist = xmlDoc.getElementsByTagName("a");
								for (i=0;i<aalist.length;i++) {
									xhr.resources.push(aalist[i].innerHTML);
								}
							}
						}
						xhr.open('GET', resources_url, false);
						xhr.send();
					...
					function urlData(url) {
						...
						xhd.onload = function() {
							if (xhd.status != 404 && xhd.readyState === xhd.DONE && xhd.status === 200) {
								xhd.base64 = xhd.response;
							} else {
								xhd.base64 = false;
							}
						}
						xhd.open('GET', url, false);
						xhd.send();
						return xhd.base64
					}
					// msx
					alt = win.find("#alt")[0].value();
					alt_msx = _("Are you sure you want to continue without including an Image Description? \
								Without it the image may not be accessible to some users with disabilities, \
					...
					// Function to check if exist alt value [windowManager]
					function checkAltAndResources() {
						var alt = win.find("#alt")[0].value();
						if (alt=="") {
							editor.windowManager.confirm(
								alt_msx,
								function(s){
									if (s) {checkResourcesAndSubmit()()}
								});
						} else {
							checkResourcesAndSubmit()();
						}
					}
					// Rename [windowManager]
					function renameFile(errorMsx='') {
						editor.windowManager.open({
							...
							body: [
								...
								},
								{
									...
									name: 'newName',
									size: 30,
									value: preview_basename.split('.')[0],
								}
							],
							buttons: [
								{
							...
							onsubmit: function(e) {
								newNameFile = e.data.newName+"."+preview_basename.split(".")[1]
								if (e.data.newName == "") {
									e.data.errorLabel = "The text field should not be left empty.";
									renameFile(e.data.errorLabel)
								} else if (resourceList().includes(newNameFile)) {
									e.data.errorLabel = "There's already a file with that name. \
														Try again.";
			   						renameFile(e.data.errorLabel)
								} else {
									newNameFile = e.data.newName+"."+preview_basename.split(".")[1]
									window.parent.nevow_clientToServerEvent(
										'EditBasename', this, '', previews_base, newNameFile);
									editor.windowManager.close();
									onSubmitForm();
								}
							}
					...
					// Panel to decide what to do with the two images [windowManager]
					function panelCompareImg() {
						editor.windowManager.open({
							title: 'Warning',
							close_previous : "no",
							body: [
								{
									type: 'panel',
									label: _("There's already a file with that name.")
								},
								{
									type: 'button',
									name: 'img1',
									...
									onclick: function() {editor.windowManager.open({
											title: _('New Image'),
											url: previews_img,
											height: metaImgPreview.naturalHeight+2,
											width: metaImgPreview.naturalWidth+2,
											close_previous : "no",
										}
									)}
								},
								{
									type: 'button',
									name: 'img2',
									label: _('Package Image:'),
									icon: 'image',
									select: false,
									onclick: function() {editor.windowManager.open({
											title: _('Package Image'),
											url: resource_img,
											height: metaImgResource.naturalHeight+2,
											width: metaImgResource.naturalWidth+2,
											close_previous : "no"
										}
									)}
								},
							...
							buttons: [
								{
									...
									onclick: function() {
										window.parent.nevow_clientToServerEvent(
											'RemoveResourceFile', this, '', preview_basename);
										editor.windowManager.close();
										onSubmitForm();
									}
								},
								{
									text: _('Rename the new file'),
									name: 'rename',
									onclick: function(){
										editor.windowManager.close();
										renameFile();
					...
					// Check if there is a file with the same name in resources
					function checkResourcesAndSubmit() {
						if (win.find("#src")[0].value().includes("/previews/")) {
							if (resourceList().includes(preview_basename)) {
								if (urlData(previews_img) != urlData(resource_img)) {
									panelCompareImg();
								} else {
									onSubmitForm();
								}
					...
					// Checks and submit
					var preview_basename = previewBasename();
					var resource_img = resources_url+"/"+preview_basename;
					var metaImgPreview = new Image();
					metaImgPreview.src = previews_img;
					var metaImgResource = new Image();
					metaImgResource.src = resource_img;
					checkAltAndResources();
					return false;
				}
				// / The New eXeLearning
			});
