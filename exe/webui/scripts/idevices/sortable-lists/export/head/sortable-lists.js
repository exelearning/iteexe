		...
		if (lists.length>0) {
			...
			lists.each(
				function(i){
					...
					$exeSortableLists.getListHTML(lis,this,i);
				}
			);
			var style = '\
			<style type="text/css">\
				ul.exe-sortableList{margin:0;padding:0;list-style:none}\
				ul.exe-sortableList li{background:#FFF;color:#333;padding:.5em 65px .5em 10px;margin:0 0 .5em 0!important;border:1px solid #CCC;border-radius:4px;;list-style-image:none!important}\
			</style>\
			';
			$("HEAD").append(style)
		}
	},
	getListHTML : function(lis,list,listOrder) {
		var html = '<ul class="exe-sortableList" id="exe-sortableList-'+listOrder+'">';
		for (var i=0;i<lis.length;i++) {
			html += '<li>'+lis[i]+'</li>';
		}
		html += "</ul>";
		html += '<p><input type="button" class="feedbackbutton" value="Comprobar" onclick="$exeSortableLists.check('+listOrder+')" /></p>';
		html += '<div id="exe-sortableList-'+listOrder+'-feedback"></div>';
		html = $(html);
		$(list).hide().attr("id","exe-sortableListResults-"+listOrder).before(html);
		html.sortable();
	},
	check : function(listOrder){
		var right = true;
		...
		if (right) feedback.html("<p>¡Correcto!</p>");
		else feedback.html("<p>No es correcto&hellip;<p><p><strong>Respuesta correcta:</strong></p><ul>"+rightAnswers.html()+"</ul>");
	},
