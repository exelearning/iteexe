...
var TinyMCE_XHTMLXtrasPlugin = {
	...
	getControlHTML : function(cn) {
		switch (cn) {
			...
			case "glosario":
				return tinyMCE.getButtonHTML(cn, 'lang_xhtmlxtras_glosario_desc', '{$pluginurl}/images/glosario.png', 'mceGlosario', true);
			case "codigo":
				return tinyMCE.getButtonHTML(cn, 'lang_xhtmlxtras_codigo_desc', '{$pluginurl}/images/codigo.png', 'mceCodigo', true);
			case "idioma":
				return tinyMCE.getButtonHTML(cn, 'lang_xhtmlxtras_idioma_desc', '{$pluginurl}/images/idioma.png', 'mceIdioma', true);
		}
		...
		switch (command) {
			...
			case "mceGlosario":
				if (!this._anySel(editor_id))
					return true;
				template = new Array();
				template['file'] = '../../plugins/xhtmlxtras/glosario.htm';
				template['width'] = 350;
				template['height'] = 170;
				tinyMCE.openWindow(template, {editor_id : editor_id});
			...
			case "mceCodigo":
				template = new Array();
				template['file'] = '../../plugins/xhtmlxtras/codigo.htm';
				template['width'] = 350;
				template['height'] = 250;
				tinyMCE.openWindow(template, {editor_id : editor_id});
			...
			case "mceIdioma":
				...
				template = new Array();
				template['file'] = '../../plugins/xhtmlxtras/idioma.htm';
				template['width'] = 350;
				template['height'] = 250;
				tinyMCE.openWindow(template, {editor_id : editor_id});
				return true;
	...
	handleNodeChange : function(editor_id, node, undo_index,undo_levels, visual_aid, any_selection) {
		...
		if (!any_selection) {
			...
			tinyMCE.switchClass(editor_id + '_glosario', 'mceButtonDisabled');
			tinyMCE.switchClass(editor_id + '_codigo', 'mceButtonNormal');
			tinyMCE.switchClass(editor_id + '_idioma', 'mceButtonDisabled');
		} else {
			...
			tinyMCE.switchClass(editor_id + '_glosario', 'mceButtonNormal');
			tinyMCE.switchClass(editor_id + '_coddigo', 'mceButtonNormal');
			tinyMCE.switchClass(editor_id + '_idioma', 'mceButtonNormal');
		}
		...
		switch (node.nodeName) {
			...
			case "INS":
				...
				return true;
			case "A":
				if (elm.className == 'glosario')
					tinyMCE.switchClass(editor_id + '_glosario', 'mceButtonSelected');
				return true;
			case "DIV":
				if (elm.className.indexOf('codigo') != -1)
					tinyMCE.switchClass(editor_id + '_codigo', 'mceButtonSelected');
				return true;
			case "CODE":
				tinyMCE.switchClass(editor_id + '_codigo', 'mceButtonSelected');
				return true;
			case "SPAN":
				tinyMCE.switchClass(editor_id + '_idioma', 'mceButtonSelected');
				return true;
		}
	...
	_anySel : function(editor_id) {
		...
		pe = tinyMCE.getParentElement(inst.getFocusElement(), 'CITE,ACRONYM,ABBR,HTML:ABBR,DEL,INS,A,DIV,CODE,SPAN');
