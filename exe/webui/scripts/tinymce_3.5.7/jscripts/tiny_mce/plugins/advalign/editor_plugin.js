	...
	isBlock : function(editorID,currentNode,command){
		...
		e.each(function(){
			if(this.className && this.className.indexOf("exe-figure")==0) {
				...
				tinyMCE.activeEditor.controlManager.get("justifyleft").setActive(false);
				tinyMCE.activeEditor.controlManager.get("justifycenter").setActive(false);
				tinyMCE.activeEditor.controlManager.get("justifyright").setActive(false);				
				...
				if (currK.indexOf(" "+k)!=-1) {
					...
					currK += " "+k;
					tinyMCE.activeEditor.controlManager.get(pos).setActive(true);
				}
