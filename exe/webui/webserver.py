...
try:
    ...
    # expand the above for the port testing:
...
class WebServer:
    ...
    def find_port(self):
        ...
        while not port_test_done:
            ...
            try:
                ...
                last_exception = exc
                ###########################
                # Since we can connect to this port, see if it's already running an eXe server, as we only want 1 running at a time:
                #
                ...
                # unset any blocking:
                s.setblocking(0)
                # set timeout to reasonably "large enough", but not "too large" (note that google, e.g., sometimes requires 3secs):
    ...
    def run(self):
        ...
        # A port for this server was looked for earlier by find_port.  Ensure that it is valid (>= 0):
